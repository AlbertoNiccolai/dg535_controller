...
class MainWindow(tk.Tk):
    ...
    def setup_ui(self):
        ...
        self.trigger_mode_entry = self.create_labeled_entry(settings_frame, "Trigger Mode")
        ...
        # Additional buttons for setting delays
        delay_button_frame = ttk.Frame(general_frame)
        delay_button_frame.pack(pady=10)

        set_delay_t0_button = ttk.Button(delay_button_frame, text="Set Delay T0", command=lambda: self.write_on_dg535_delay('t0'))
        set_delay_t0_button.pack(side=tk.LEFT, padx=5)

        set_delay_a_button = ttk.Button(delay_button_frame, text="Set Delay A", command=lambda: self.write_on_dg535_delay('a'))
        set_delay_a_button.pack(side=tk.LEFT, padx=5)

        set_delay_b_button = ttk.Button(delay_button_frame, text="Set Delay B", command=lambda: self.write_on_dg535_delay('b'))
        set_delay_b_button.pack(side=tk.LEFT, padx=5)

        set_delay_c_button = ttk.Button(delay_button_frame, text="Set Delay C", command=lambda: self.write_on_dg535_delay('c'))
        set_delay_c_button.pack(side=tk.LEFT, padx=5)

        set_delay_d_button = ttk.Button(delay_button_frame, text="Set Delay D", command=lambda: self.write_on_dg535_delay('d'))
        set_delay_d_button.pack(side=tk.LEFT, padx=5)
    ...
    def write_on_dg535_tm(self):
        trigger_mode_line = self.trigger_mode_entry.get()  # Get trigger mode input
